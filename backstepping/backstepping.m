...
Reqd(:,1) = [ 0, 10, 0, 0.099, 0, 0, 0.1, 0, 0, -0.001];    % only 1,2,4,7,8 are specified here
...
for i = 1:j
    disp(i);
    vp = sqrt( Reqd(7,i)*Reqd(7,i)+ Reqd(8,i)*Reqd(8,i) );
    beta = angwrapfn(atan2(Reqd(8,i),Reqd(7,i)));  % beta in range ( 0 to 6.28 )        '
    beta_dot = ((Reqd(7,i)*Reqd(10,i)) - (Reqd(8,i)*Reqd(9,i)))/(vp*vp);
    vp_dot = ((Reqd(7,i)*Reqd(9,i)) + (Reqd(8,i)*Reqd(10,i)))/vp;
    ...
    Reqd(5,i) = sqrt( (vp*vp) - (Reqd(4,i)*Reqd(4,i)) );
    ...
    % Position Error
    Error(1:6,i) = auv(1:6,i) - Reqd(1:6,i); 
    ...
    % Updating Input Force and Torque
    Fin(1) = Freq(1) + (d(1)*Error(4,i)) - ( m(2)*(Error(5,i)*(Error(6,i)+Reqd(6,i)) + Reqd(5,i)*Error(6,i)) ) + m(1)*Tu;      
    Fin(2) = Freq(2) + (d(3)*Error(6,i)) + ( (m(2)-m(1)) * ( Error(4,i)*(Error(5,i)+Reqd(5,i)) + Reqd(4,i)*Error(5,i) ) ) + m(3)*Tr;
    ...
    Reqd(1:2,i+1) = [ q(end,1:2)];
    Reqd(7:10,i+1) = [ q(end,3:6)];
    n=n+0.15;  
    
    % update Reqd surge velocity
    [t3,ur] = ode45('auv_surge',time_period,Reqd(4,i));    
    Reqd(4,i+1) = ur(end,1);
